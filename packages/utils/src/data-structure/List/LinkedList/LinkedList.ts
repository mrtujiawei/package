...
class LinkedList<T> {
   ...
   * 移除满足条件的节点
   */
  removeEach(callback: (value: T, index: number, context: LinkedList<T>) => boolean) {
   ...
   * 遍历并过滤
   */
  filter(callback: (value: T, index: number, context: LinkedList<T>) => boolean) {
